...
class LeffaVirtualTryOn:
    def __init__(self, ckpt_dir: str):
        ...
        controlnet = ControlNetModel.from_single_file(
            f"{ckpt_dir}/controlnetFor_v10.safetensors",
    ...
    def generate_skin(
        ...
        skin_prompt = "Wearing Held Tight Short Sleeve Shirt, high quality skin, realistic, high quality, Neat attire"
        ...
        # Generate DensePose control image
        densepose_result = self.densepose_predictor.predict_seg(np.array(src_image))
        densepose_image = Image.fromarray(densepose_result[:, :, ::-1])

        # 검증: 이미지가 정상적으로 반환되었는지 확인
        if not isinstance(densepose_image, Image.Image):
            raise TypeError(f"DensePose에서 반환된 control image가 유효하지 않습니다: {type(densepose_image)}")
        ...
        generated_image = self.skin_pipe(
            ...
            control_image=densepose_image,
            width=src_image.width,
            height=src_image.height,
            num_inference_steps=step,
            generator=generator,
            guidance_scale=3.0  # Lower guidance to better match image context
        ).images[0]
        ...
        return final_image, densepose_image
    ...
    def skin_test(
        ...
        # 손 마스크를 추가하여 인페인팅할 마스크 계산
        hands_mask_np = np.isin(parsing_map, [14, 15]).astype(np.uint8)  # 손
        inpaint_mask_np = (garment_mask_np | hands_mask_np) & limb_mask_np

        # 5. 마스크에 10px 마진 추가 (팽창)
        kernel = np.ones((0, 0), np.uint8)  # 10px 마진용 커널
        ...
        # 6. 피부 생성 호출
        final_image, densepose_image = self.generate_skin(
            src_image=src_image,
        ...
        return final_image, inpaint_mask_img, densepose_image
    ...
    def leffa_predict(
        ...
        if control_type == "virtual_tryon":
            ...
            mask = Image.fromarray(np.ones_like(np.array(agnostic_image), dtype=np.uint8) * 255)
